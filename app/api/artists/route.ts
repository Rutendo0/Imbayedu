...
export async function GET() {
  ...
  if (!artists || artists.length === 0) {
    try {
      // Check if artists already exist before creating to prevent duplicates
      const existingTunga = artists.find(a => a.name === "Tunga Makoni")
      const existingOneal = artists.find(a => a.name === "O'Neal Tanaka Maisiri ")
  ...
  console.log('Returning', artists?.length || 0, 'artists from API')
